<template>
  <div>
    <Container
      v-if="!loading"
      class="h-full flex overflow-x-auto gap-8 bg-white outline outline-orange400/20 outline-2 rounded-lg p-2"
      group-name="cols"
      tag="div"
      orientation="horizontal">
      <Draggable class=" bg-orange400/20 rounded-lg h-full w-96 flex-shrink-0 shadow-lg"
        v-for="column in columnNames" :key="column.id">
        <div class="h-full flex flex-col">

          <!-- header-->
          <div class="cursor-move rounded-t-lg p-4 space-x-4 bg-primary text-black flex">
            <span class="text-lg font-bold">{{ column.name }}</span>
          </div>
          <!-- column -->
          <Container
...
import { applyDrag } from '../../../helpers/Vue3Smooth.js'
